 ...
 */
class ArrayAvg extends UserDefinedAggregateFunction {
  ...
  def bufferSchema: StructType = StructType(
    StructType(Array(StructField("append", ArrayType(StringType), true)))
  )
  ...
  def update(buffer: MutableAggregationBuffer, input: Row): Unit = {
    val a= input.getString(0)
    val b = buffer.getSeq[String](0)

    buffer(0) = if(b.contains(a)){b} else b.+:(a)
  ...
  def merge(buffer1: MutableAggregationBuffer, buffer2: Row): Unit = {
    val a= buffer1.getSeq[String](0)
    val b = buffer2.getSeq[String](0)
    val c =a ++ b
    buffer1(0) = c.toSet.filter(_ !="").toArray
  }

  // 最后，指的是，一个分组的聚合值，如何通过中间的缓存聚合值，最后返回一个最终的聚合值
  def evaluate(buffer: Row): Any = {

    buffer.getSeq[String](0)
